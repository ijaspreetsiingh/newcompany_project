...
class LandingController extends Controller
{
    ...
    public function home()
    {
        ...
        $settings = cache()->remember('landing_settings', 3600, fn() => $this->businessSettings->whereNotIn('settings_type', ['payment_config', 'third_party'])->get());
        $settingss = cache()->remember('landing_data_settings', 3600, fn() => $this->dataSetting->whereIn('type', ['landing_web_app', 'landing_text_setup'])->get());
        $categories = cache()->remember('landing_categories', 3600, fn() => $this->category->ofType('main')->ofStatus(1)->with(['children'])->withCount('zones')->get());
        $testimonials = cache()->remember('landing_testimonials', 3600, fn() => LandingPageTestimonial::all());
        $features = cache()->remember('landing_features', 3600, fn() => LandingPageFeature::all());
        $specialities = cache()->remember('landing_specialities', 3600, fn() => LandingPageSpeciality::all());
