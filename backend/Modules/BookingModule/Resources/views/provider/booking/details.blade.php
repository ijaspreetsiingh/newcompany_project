                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="py-3 d-flex flex-column gap-3 mb-2">
                                ...
                                @if(in_array($booking->booking_status, ['ongoing']) && isset($booking->serviceman))
                                <div class="c1-light-bg radius-10">
                                    <div class="border-bottom d-flex align-items-center justify-content-between gap-2 py-3 px-4 mb-2">
                                        <h4 class="d-flex align-items-center gap-2">
                                            <span class="material-icons title-color">location_on</span>
                                            {{ translate('Serviceman_Live_Tracking') }}
                                        </h4>
                                        <span class="badge badge-warning" id="tracking-status">{{ translate('Tracking') }}</span>
                                    </div>
                                    <div class="py-3 px-4">
                                        <div id="serviceman-tracking-map" style="height: 350px; border-radius: 10px; z-index: 1;"></div>
                                        <div class="d-flex justify-content-between mt-2" id="tracking-info" style="display:none;">
                                            <small class="text-muted"><span class="material-icons" style="font-size:14px;">schedule</span> <span id="tracking-updated-at"></span></small>
                                            <small class="text-muted"><span id="tracking-distance"></span></small>
                                        </div>
                                    </div>
                                </div>
                                @endif
    ...
    <script>
        ...
        $(document).ready(function() {
            function addressMap() {
                ...
                const map = L.map("address_location_map_canvas").setView(myLatLng, 13);
                    ...
                    fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat=' + latlng.lat + '&lon=' + latlng.lng)
                        ...
                        .then(function(data) {
                            if (data && data.display_name) {
                                document.getElementById('address_address').value = data.display_name;
                            }
                        });
                });
                                ...
                                if (data && data.length > 0) {
                                    data.forEach(function(place) {
                                        ...
                                        mrkr.on('click', function() {
                                            document.getElementById('address_latitude').value = place.lat;
                                            document.getElementById('address_longitude').value = place.lon;
                                        });
        ...
        $(document).ready(function() {
            ...
            (function() {
                const bookingId = '{{ $booking->id }}';
                const serviceLat = {{ $booking?->service_address?->lat ?? 'null' }};
                const serviceLng = {{ $booking?->service_address?->lon ?? 'null' }};
                const locationUrl = '{{ route("provider.booking.serviceman_location", $booking->id) }}';

                let trackingMap = null;
                let servicemanMarker = null;
                let serviceMarker = null;
                let routeLine = null;
                let trackingInterval = null;

                function initTrackingMap() {
                    const container = document.getElementById('serviceman-tracking-map');
                    if (!container) return;

                    const defaultCenter = (serviceLat && serviceLng) ? [serviceLat, serviceLng] : [23.8103, 90.4125];

                    trackingMap = L.map('serviceman-tracking-map', {
                        zoomControl: false
                    ...
                    if (serviceLat && serviceLng) {
                        serviceMarker = L.marker([serviceLat, serviceLng], {
                            icon: L.divIcon({
                                className: 'custom-marker',
                                html: '<div style="background:#4CAF50;width:24px;height:24px;border-radius:50%;border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;"><span class="material-icons" style="color:white;font-size:14px;">home</span></div>',
                                iconSize: [24, 24],
                                iconAnchor: [12, 12]
                            })
                        }).addTo(trackingMap);
                    ...
                    fetchServicemanLocation();
                    trackingInterval = setInterval(fetchServicemanLocation, 5000);
                }

                function fetchServicemanLocation() {
                    fetch(locationUrl)
                        ...
                        .then(data => {
                            if (data.lat && data.lng) {
                                ...
                                if (!servicemanMarker) {
                                    servicemanMarker = L.marker([lat, lng], {
                                        icon: L.divIcon({
                                            className: 'custom-marker',
                                            html: '<div style="background:#FF6B00;width:32px;height:32px;border-radius:50%;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;"><span class="material-icons" style="color:white;font-size:18px;">person</span></div>',
                                            iconSize: [32, 32],
                                            iconAnchor: [16, 16]
                                        })
                                    }).addTo(trackingMap);
                                } else {
                                    servicemanMarker.setLatLng([lat, lng]);
                                }

                                document.getElementById('tracking-info').style.display = 'flex';
                                if (data.last_updated_at) {
                                    document.getElementById('tracking-updated-at').textContent = data.last_updated_at;
                                }

                                if (serviceLat && serviceLng) {
                                    const dist = getDistance(lat, lng, serviceLat, serviceLng);
                                    document.getElementById('tracking-distance').textContent = dist < 1 ? Math.round(dist * 1000) + ' m' : dist.toFixed(1) + ' km';
                                }

                                if (!routeLine && serviceLat && serviceLng) {
                                    fetchRoute(lat, lng, serviceLat, serviceLng);
                                } else if (routeLine) {
                                    updateRoute(lat, lng, serviceLat, serviceLng);
                                }
                            } else {
                                document.getElementById('tracking-status').textContent = '{{ translate("No_Location_Data") }}';
                                document.getElementById('tracking-status').className = 'badge badge-secondary';
                            }
                        })
                        .catch(err => console.log('Tracking fetch error:', err));
                }

                function fetchRoute(oLat, oLng, dLat, dLng) {
                    fetch('https://router.project-osrm.org/route/v1/driving/' + oLng + ',' + oLat + ';' + dLng + ',' + dLat + '?geometries=geojson&overview=full')
                        .then(response => response.json())
                        .then(data => {
                            if (data.code === 'Ok' && data.routes && data.routes.length > 0) {
                                const coords = data.routes[0].geometry.coordinates.map(c => [c[1], c[0]]);
                                routeLine = L.polyline(coords, { color: '#1976D2', weight: 5, opacity: 0.8 }).addTo(trackingMap);

                                const bounds = L.latLngBounds([[oLat, oLng], [dLat, dLng]]);
                                routeLine.getLatLngs().forEach(p => bounds.extend(p));
                                trackingMap.fitBounds(bounds, { padding: [50, 50] });
                            }
                        });
                }

                function updateRoute(oLat, oLng, dLat, dLng) {
                    fetch('https://router.project-osrm.org/route/v1/driving/' + oLng + ',' + oLat + ';' + dLng + ',' + dLat + '?geometries=geojson&overview=full')
                        .then(response => response.json())
                        .then(data => {
                            if (data.code === 'Ok' && data.routes && data.routes.length > 0) {
