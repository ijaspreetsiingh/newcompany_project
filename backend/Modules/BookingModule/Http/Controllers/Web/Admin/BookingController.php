...
class BookingController extends Controller
{
    ...
    public function index(Request $request): Renderable
    {
        ...
        // Build a unique cache key based on current user, role, and request parameters
        $user = auth()->user();
        $role = $user->role ?? 'admin';
        $cacheKey = "booking_index_{$role}_{$user->id}_" . md5(serialize($request->except(['_token'])));

        $bookings = Cache::remember($cacheKey, 60, function () use ($request, $bookingStatus, $maxBookingAmount, $queryParams) {
            return $this->booking
                ->with([
                    // Eager load only needed columns to reduce payload
                    'customer:id,first_name,last_name,email,phone',
                    'provider:id,company_name,company_phone',
                    'serviceman:id',
                    'repeat',
                ])
    ...
    public function bookingVerificationList(Request $request): Factory|View|Application
    {
        ...
        $bookings = $this->booking->with(['customer', 'repeat'])
    ...
    public function servicemanLocation(Request $request, string $booking_id): JsonResponse
    {
        $booking = $this->booking->with(['serviceman.user'])->find($booking_id);
        ...
        if (!isset($servicemanUser)) {
            return response()->json(['lat' => null, 'lng' => null, 'last_updated_at' => null]);
        }

        return response()->json([
            'lat' => $servicemanUser->current_lat,
            'lng' => $servicemanUser->current_lng,
            'last_updated_at' => $servicemanUser->last_location_updated_at,
        ]);
    }
