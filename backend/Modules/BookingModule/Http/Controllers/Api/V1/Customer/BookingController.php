...
class BookingController extends Controller
{
    ...
    /**
     * Get serviceman's current GPS location for a booking
     * @param Request $request
     * @param string $booking_id
     * @return JsonResponse
     */
    ...
    {
        $booking = $this->booking->with(['detail', 'serviceman.user'])->where('id', $booking_id)->first();

        if (!isset($booking)) {
            return response()->json(response_formatter(DEFAULT_404), 404);
        }

        if ($this->isCustomerLoggedIn && $booking->customer_id !== $this->customerUserId) {
        ...
        return response()->json(response_formatter(DEFAULT_200, $location), 200);
    }
