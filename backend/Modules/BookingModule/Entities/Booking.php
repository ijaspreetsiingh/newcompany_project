...
class Booking extends Model
{
    ...
    protected $casts = [
        ...
        'auto_assign_expires_at' => 'datetime',
        'serviceman_assign_expires_at' => 'datetime',
    ...
    public static function boot()
    {
        ...
        self::creating(function ($model) {
            $model->readable_id = ($model->max('readable_id') ?? 99999) + 1;
        });
        ...
        self::updating(function ($model) {
            ...
            if ($model->isDirty('booking_status')) {
                ...
                if ($model?->provider) {
                    ...
                    if ($limit_status == '100_percent' && business_config('suspend_on_exceed_cash_limit_provider', 'provider_config')->live_values) {
                        ...
                        if ($emailStatus){
                            try {
                                Mail::to($provider?->owner?->email)->queue(new CashInHandOverflowMail($provider));
            ...
            if (isset($booking_notification_status) && $booking_notification_status['push_notification_booking'] && !empty($notifications)) {
                \App\Jobs\SendBookingNotificationsJob::dispatch($notifications, $model->id, 'updating', $model->provider_id);
            }
        }
        });
        ...
        self::updated(function ($model) {
            ...
            if (isset($booking_notification_status) && $booking_notification_status['push_notification_booking'] && !empty($notifications)) {
                \App\Jobs\SendBookingNotificationsJob::dispatch($notifications, $model->id, 'updated', $model->provider_id);
            }
        });
