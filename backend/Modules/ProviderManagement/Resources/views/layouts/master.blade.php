...
<main class="main-area" id="app-content-wrapper">
    <div id="app-content">
    @yield('content')
    </div>
                ...
                @if(!empty($fcm_credentials))
                    ...
                    function startFCM() {
                        messaging
                            ...
                            .then(function(token) {
                                subscribeTokenToBackend(token, 'jdds_provider_{{auth()->user()->provider->zone_id}}_{{ auth()->user()->provider->id }}_booking_message');
                                @if($serviceAtProviderPlace)
                                    @if(in_array('customer', $serviceLocations))
                                        subscribeTokenToBackend(token, 'jdds_provider_{{auth()->user()->provider->zone_id}}_customer_booking_message');
                                   @endif
                                   @if(in_array('provider', $serviceLocations))
                                        subscribeTokenToBackend(token, 'jdds_provider_{{auth()->user()->provider->zone_id}}_provider_booking_message');
                                   @endif
                                @else
                                   subscribeTokenToBackend(token, 'jdds_provider_{{auth()->user()->provider->zone_id}}_booking_message');
                                @endif
...
<script>
    ...
    $(document).on('click', '.aside .nav a[href]', function(e) {
        var $link = $(this);
        var href = $link.attr('href');
        if (!href || href === '#' || href === 'javascript:void(0)' || href.indexOf('javascript:') === 0) return;
        if (e.ctrlKey || e.metaKey || e.shiftKey) return;
        var $parent = $link.closest('li');
        var hasSubItems = $parent.hasClass('has-sub-item');
        var inSubmenu = $link.closest('.sub-menu').length > 0;
        if (hasSubItems && !inSubmenu) return;
        e.preventDefault();
        if (isNavigating) return;
        isNavigating = true;
        var $content = $('#app-content');
        $content.css('opacity', '0.5').css('transition', 'opacity 0.15s');
        $.ajax({
            url: href,
            type: 'GET',
            ...
            success: function(html) {
                var parsed = $(html);
                var newContent = parsed.find('#app-content').html() || parsed.filter('#app-content').html() || html;
                var newTitle = parsed.filter('title').text() || document.title;
                ...
                if (typeof $.fn.select2 !== 'undefined') {
                    $('.js-select, .select2').select2();
                }
                $(document).trigger('ajaxContentLoaded');
                $('.aside .nav a').removeClass('active-menu');
