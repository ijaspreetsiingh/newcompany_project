...
$booking = null; // not needed anymore
$maxBookingAmount = (business_config('max_booking_amount', 'booking_setup'))->live_values;
$subscribed_sub_category_ids = \Modules\ProviderManagement\Entities\SubscribedService::where(['provider_id' => $provider_id])->ofSubscription(1)->pluck('sub_category_id')->toArray();
$serviceAtProviderPlace = (int)((business_config('service_at_provider_place', 'provider_config'))->live_values ?? 0);
$serviceLocations = getProviderSettings(providerId: $provider_id, key: 'service_location', type: 'provider_config') ?? ['customer'];

$pending_booking_count = Cache::remember("provider_{$provider_id}_sidebar_pending_count", 60, function () use ($provider_id, $maxBookingAmount, $serviceAtProviderPlace, $serviceLocations) {
    return \Modules\BookingModule\Entities\Booking::providerPendingBookings(auth()->user()->provider, $maxBookingAmount)
        ->when($serviceAtProviderPlace == 1, function ($query) use ($serviceLocations) {
            $query->whereIn('service_location', $serviceLocations);
        })->whereDoesntHave('ignores', function ($query) use ($provider_id) {
            $query->where('provider_id', $provider_id);
        })
        ->count();
});
$accepted_booking_count = Cache::remember("provider_{$provider_id}_sidebar_accepted_count", 60, function () use ($provider_id, $maxBookingAmount) {
    return \Modules\BookingModule\Entities\Booking::providerAcceptedBookings($provider_id, $maxBookingAmount)->count();
});
$ongoing_booking_count = Cache::remember("provider_{$provider_id}_sidebar_ongoing_count", 60, function () use ($provider_id) {
    return \Modules\BookingModule\Entities\Booking::where('provider_id', $provider_id)->where('booking_status', 'ongoing')->count();
});
$completed_booking_count = Cache::remember("provider_{$provider_id}_sidebar_completed_count", 60, function () use ($provider_id) {
    return \Modules\BookingModule\Entities\Booking::where('provider_id', $provider_id)->where('booking_status', 'completed')->count();
});
$canceled_booking_count = Cache::remember("provider_{$provider_id}_sidebar_canceled_count", 60, function () use ($provider_id) {
    return \Modules\BookingModule\Entities\Booking::where('provider_id', $provider_id)->where('booking_status', 'canceled')->count();
});
...
<aside class="aside">
    <div class="aside-header">
        <a href="{{route('provider.dashboard')}}" class="logo d-flex gap-2">
            <img src="{{ $provider->logo_full_path ?: $logo }}"
    ...
    <div class="aside-body" data-trigger="scrollbar">
        ...
        <ul class="nav">
                        ...
                        <a href="{{route('provider.booking.list', ['booking_status'=>'ongoing','service_type'=>'all'])}}"
                           class="{{request()->is('provider/booking/list') && request()->query('booking_status')=='ongoing'?'active-menu':''}}">
                            <span class="link-title">{{translate('Ongoing')}}
                                <span class="count">{{$ongoing_booking_count}}</span>
                            </span>
                        </a>
                    </li>
                    <li>
                        <a href="{{route('provider.booking.list', ['booking_status'=>'completed','service_type'=>'all'])}}"
                           class="{{request()->is('provider/booking/list') && request()->query('booking_status')=='completed'?'active-menu':''}}">
                            <span class="link-title">{{translate('Completed')}}
                                <span class="count">{{$completed_booking_count}}</span>
                            </span>
                        </a>
                    </li>
                    <li>
                        <a href="{{route('provider.booking.list', ['booking_status'=>'canceled','service_type'=>'all'])}}"
                           class="{{request()->is('provider/booking/list') && request()->query('booking_status')=='canceled'?'active-menu':''}}">
                            <span class="link-title">{{translate('Canceled')}}
                                <span class="count">{{$canceled_booking_count}}</span>
                            </span>
                        </a>
                    </li>
                </ul>
            ...
            <li>
                <a href="{{route('provider.auto_assign_settings')}}"
                   class="{{request()->is('provider/auto-assign-settings')?'active-menu':''}}">
                    <span class="material-symbols-outlined" title="Auto Assign Settings">sync_alt</span>
                    <span class="link-title">{{translate('auto_assign_settings')}}</span>
                </a>
            </li>
            <li>
