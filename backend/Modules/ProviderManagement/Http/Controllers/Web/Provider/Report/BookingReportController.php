...
class BookingReportController extends Controller
{
    ...
    public function getBookingReport(Request $request): Renderable
    {
        ...
        //** Card Data **
        $filteredAmountQuery = self::filterQuery($this->booking, $request)
            ->whereIn('booking_status', ['accepted', 'ongoing', 'completed', 'canceled']);

        $bookingsCount = [];
        $bookingsCount['total_bookings'] = (clone $filteredAmountQuery)->count();
        $bookingsCount['accepted'] = (clone $filteredAmountQuery)->where('booking_status', 'accepted')->count();
        $bookingsCount['ongoing'] = (clone $filteredAmountQuery)->where('booking_status', 'ongoing')->count();
        $bookingsCount['completed'] = (clone $filteredAmountQuery)->where('booking_status', 'completed')->count();
        $bookingsCount['canceled'] = (clone $filteredAmountQuery)->where('booking_status', 'canceled')->count();

        $bookingAmount = [];
        $bookingAmount['total_booking_amount'] = (clone $filteredAmountQuery)->sum('total_booking_amount');
        $bookingAmount['total_paid_booking_amount'] = (clone $filteredAmountQuery)->where('payment_method', '!=', 'cash_after_service')->where('booking_status', 'completed')->sum('total_booking_amount');
        $bookingAmount['total_unpaid_booking_amount'] = (clone $filteredAmountQuery)->where('payment_method', '!=', 'cash_after_service')->where('booking_status', '!=', 'completed')->sum('total_booking_amount');
