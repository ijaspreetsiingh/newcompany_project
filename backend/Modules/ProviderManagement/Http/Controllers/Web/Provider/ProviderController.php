...
class ProviderController extends Controller
{
    ...
    /**
     * AUTO-ASSIGN: settings page for provider web panel
     * @param Request $request
     * @return Renderable
     */
    public function autoAssignSettings(Request $request): Renderable
    {
        $provider = $this->provider->where('user_id', $request->user()->id)->first();
        return view('providermanagement::auto-assign-settings', compact('provider'));
    }

    /**
     * AUTO-ASSIGN: update toggle & wait time
     * @param Request $request
     * @return RedirectResponse
     */
    public function updateAutoAssignSettings(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'auto_assign_mode' => 'required|in:0,1',
            'auto_assign_wait_time' => 'required|integer|min:30|max:600',
        ]);

        if ($validator->fails()) {
            Toastr::error(translate('invalid_settings_provided'));
            return back();
        }

        $provider = $this->provider->where('user_id', $request->user()->id)->first();
        $provider->auto_assign_mode = (int)$request->auto_assign_mode;
        $provider->auto_assign_wait_time = (int)$request->auto_assign_wait_time;
        $provider->save();
