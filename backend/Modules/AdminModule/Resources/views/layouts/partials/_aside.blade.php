...
$pending_booking_count = Cache::remember('sidebar_pending_booking_count', 60, function () use ($max_booking_amount) {
    return \Modules\BookingModule\Entities\Booking::where('booking_status', 'pending')
...
$offline_booking_count = Cache::remember('sidebar_offline_booking_count', 60, function () {
    return \Modules\BookingModule\Entities\Booking::whereIn('booking_status', ['pending', 'accepted'])
        ->where('payment_method', 'offline_payment')->where('is_paid', 0)->count();
});

$accepted_booking_count = Cache::remember('sidebar_accepted_booking_count', 60, function () use ($max_booking_amount) {
    return \Modules\BookingModule\Entities\Booking::where('booking_status', 'accepted')
        ->when($max_booking_amount > 0, function ($query) use ($max_booking_amount) {
            $query->where(function ($query) use ($max_booking_amount) {
                $query->where('payment_method', 'cash_after_service')
                    ->where(function ($query) use ($max_booking_amount) {
...
$ongoing_booking_count = Cache::remember('sidebar_ongoing_booking_count', 60, function () {
    return \Modules\BookingModule\Entities\Booking::where('booking_status', 'ongoing')->count();
});

$completed_booking_count = Cache::remember('sidebar_completed_booking_count', 60, function () {
    return \Modules\BookingModule\Entities\Booking::where('booking_status', 'completed')->count();
});

$canceled_booking_count = Cache::remember('sidebar_canceled_booking_count', 60, function () {
    return \Modules\BookingModule\Entities\Booking::where('booking_status', 'canceled')->count();
});

$pending_providers = Cache::remember('sidebar_pending_providers', 60, function () {
    return \Modules\ProviderManagement\Entities\Provider::ofApproval(2)->count();
});
$denied_providers = Cache::remember('sidebar_denied_providers', 60, function () {
    return \Modules\ProviderManagement\Entities\Provider::ofApproval(0)->count();
});
    ...
    <div class="aside-body" data-trigger="scrollbar">
        ...
        <ul class="nav">
                            ...
                            <a href="{{route('admin.booking.post.list', ['type'=>'all'])}}"
                               class="{{request()->is('admin/booking/post') || request()->is('admin/booking/post/details*') ? 'active-menu' : ''}}">
                                <span class="link-title">{{translate('Customized_Requests')}}
                                    <span
                                        class="count">{{Cache::remember('sidebar_post_booked_count', 60, function() { return \Modules\BidModule\Entities\Post::where('is_booked', 0)->count() ?? 0; })}}</span>
                                </span>
                            </a>
                        </li>
                        <li>
                            <a href="{{route('admin.booking.list.verification', ['booking_status'=>'pending', 'type' => 'pending'])}}"
                               class="{{request()->is('admin/booking/list/verification') && request()->query('booking_status')=='pending' ?'active-menu':''}}"><span
                                    class="link-title">{{translate('verify_requests')}} <span
                                        class="count">{{Cache::remember('sidebar_verify_requests_count', 60, function() use ($max_booking_amount) { return \Modules\BookingModule\Entities\Booking::where('is_verified', '0')->where('payment_method', 'cash_after_service')->Where('total_booking_amount', '>', $max_booking_amount)->whereIn('booking_status', ['pending', 'accepted'])->count(); })}}</span></span></a>
                        </li>
                        ...
                        <li><a href="{{route('admin.booking.list', ['booking_status'=>'ongoing','service_type'=>'all'])}}"
                               class="{{request()->is('admin/booking/list') && request()->query('booking_status')=='ongoing'?'active-menu':''}}"><span
                                    class="link-title">{{translate('Ongoing')}} <span
                                        class="count">{{$ongoing_booking_count}}</span></span></a>
                        </li>
                        <li><a href="{{route('admin.booking.list', ['booking_status'=>'completed','service_type'=>'all'])}}"
                               class="{{request()->is('admin/booking/list') && request()->query('booking_status')=='completed'?'active-menu':''}}"><span
                                    class="link-title">{{translate('Completed')}} <span
                                        class="count">{{$completed_booking_count}}</span></span></a>
                        </li>
                        <li><a href="{{route('admin.booking.list', ['booking_status'=>'canceled','service_type'=>'all'])}}"
                               class="{{request()->is('admin/booking/list') && request()->query('booking_status')=='canceled'?'active-menu':''}}"><span
                                    class="link-title">{{translate('Canceled')}} <span
                                        class="count">{{$canceled_booking_count}}</span></span></a>
                        </li>
                    </ul>
                </li>
            @endcan
            ...
            @can('configuration_view')
                <li>
                    <a href="{{route('admin.configuration.get-banner-settings')}}"
