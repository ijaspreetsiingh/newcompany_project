...
<script>
   ...
   @endif
    ...
    $(document).on('click', '.aside .nav a[href]', function(e) {
        ...
        // If link is inside .sub-menu, navigate
        // If link has no sub-menu siblings, navigate
        var $parent = $link.closest('li');
        var hasSubItems = $parent.hasClass('has-sub-item');
        var inSubmenu = $link.closest('.sub-menu').length > 0;

        if (hasSubItems && !inSubmenu) {
            // This is a parent toggle link like "Bookings" - don't navigate, just toggle sub-menu
            return;
        }
        e.preventDefault();
