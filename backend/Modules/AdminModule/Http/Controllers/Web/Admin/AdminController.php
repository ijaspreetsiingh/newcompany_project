...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request, Transaction $transaction): View|Factory|Application
    {
        ...
        $completedBookingIds = BookingDetailsAmount::whereHas('booking', $baseQuery)
            ->orWhereHas('repeat', $baseQuery)
            ->select('id')
            ->getQuery();

        $aggregate = BookingDetailsAmount::whereIn('id', $completedBookingIds)
            ->first([
        ...
        $admin_commission = $aggregate->admin_commission ?? 0;
        $discount_by_admin = $aggregate->discount_by_admin ?? 0;
        $coupon_discount_by_admin = $aggregate->coupon_discount_by_admin ?? 0;
        $campaign_discount_by_admin = $aggregate->campaign_discount_by_admin ?? 0;
        ...
        $total_system_earning = DB::table('accounts')->selectRaw('SUM(received_balance + total_withdrawn) as total')->value('total') ?? 0;
        ...
        $data[] = ['top_cards' => [
            ...
            'total_system_earning' => $total_system_earning,
