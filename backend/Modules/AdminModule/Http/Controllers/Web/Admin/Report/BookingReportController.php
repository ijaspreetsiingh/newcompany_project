...
class BookingReportController extends Controller
{
    ...
    public function getBookingReport(Request $request): Renderable
    {
        ...
        $filteredQuery = self::filterQuery($this->booking, $request)
            ->whereIn('booking_status', ['pending', 'accepted', 'ongoing', 'completed', 'canceled']);

        $bookings_count = [];
        $bookings_count['total_bookings'] = (clone $filteredQuery)->count();
        $bookings_count['accepted'] = (clone $filteredQuery)->where('booking_status', 'accepted')->count();
        $bookings_count['ongoing'] = (clone $filteredQuery)->where('booking_status', 'ongoing')->count();
        $bookings_count['completed'] = (clone $filteredQuery)->where('booking_status', 'completed')->count();
        $bookings_count['canceled'] = (clone $filteredQuery)->where('booking_status', 'canceled')->count();
        $bookings_count['pending'] = (clone $filteredQuery)->where('booking_status', 'pending')->count();

        $booking_amount = [];
        $booking_amount['total_booking_amount'] = (clone $filteredQuery)->sum('total_booking_amount');
        $booking_amount['total_paid_booking_amount'] = (clone $filteredQuery)->where('payment_method', '!=', 'cash_after_service')->where('booking_status', 'completed')->sum('total_booking_amount');
        $booking_amount['total_unpaid_booking_amount'] = (clone $filteredQuery)->where('payment_method', '!=', 'cash_after_service')->where('booking_status', '!=', 'completed')->sum('total_booking_amount');
