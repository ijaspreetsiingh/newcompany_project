...
if (!function_exists('business_config')) {
    ...
    {
        $cacheKey = "bc_{$settings_type}_{$key}";
        ...
        $result = (isset($config)) ? $config : null;
...
if (!function_exists('auto_translator')) {
    ...
    {
        try {
            $res = \Illuminate\Support\Facades\Http::timeout(10)->get("https://translate.googleapis.com/translate_a/single?client=gtx&ie=UTF-8&oe=UTF-8&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&dt=at&sl=" . $sl . "&tl=" . $tl . "&hl=hl&q=" . urlencode($q));
            $data = json_decode($res->body());
            return str_replace('_', ' ', $data[0][0][0]);
        } catch (\Exception $e) {
            return $q;
        }
    }
