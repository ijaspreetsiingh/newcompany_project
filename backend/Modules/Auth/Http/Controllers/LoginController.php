...
class LoginController extends Controller
{
    ...
    public function __construct(User $user)
    {
        ...
        $this->middleware(function ($request, $next) {
            ...
            } elseif ($request->user() !== null && $request->user()->user_type === 'provider-serviceman') {
    ...
    public function servicemanLoginForm(): Application|Factory|View
    {
        return view('auth::serviceman-login');
    }

    public function servicemanPanelLogin(Request $request): RedirectResponse
    {
        $request->validate($this->validation_array);
        ...
        if (!isset($user)) {
            Toastr::error(translate(AUTH_LOGIN_404['message']));
            return redirect(route('serviceman.auth.login'));
        }

        if (!$user->is_active) {
            Toastr::error(translate(DEFAULT_USER_DISABLED_401['message']));
            return redirect(route('serviceman.auth.login'));
        }

        if (!isset($user->serviceman)) {
            Toastr::error(translate(ACCESS_DENIED['message']));
            return redirect(route('serviceman.auth.login'));
        }
        ...
        if (auth()->attempt(['email' => $request->email_or_phone, 'password' => $request->password], $remember)) {
            if ($remember) {
                cookie()->queue('serviceman_remember_email', $request->email_or_phone, 43200);
                cookie()->queue('serviceman_remember_password', $request->password, 43200);
                cookie()->queue('serviceman_remember_checked', true, 43200);
            } else {
                cookie()->queue(cookie()->forget('serviceman_remember_email'));
                cookie()->queue(cookie()->forget('serviceman_remember_password'));
                cookie()->queue(cookie()->forget('serviceman_remember_checked'));
            }
            return redirect()->route('serviceman.dashboard');
        } else {
            Toastr::error(translate(ACCESS_DENIED['message']));
            return back();
        }
    ...
    public function logout(Request $request): RedirectResponse
    {
        if (auth()->user()) {
            ...
            if (in_array($userType, ADMIN_USER_TYPES)) {
                $redirect_route = 'admin.auth.login';
            } elseif ($userType === 'provider-serviceman') {
                $redirect_route = 'serviceman.auth.login';
            } else {
                $redirect_route = 'provider.auth.login';
            }
