...
class ServiceController extends Controller
{
    ...
    public function create(Request $request): View|Factory|Application
    {
        ...
        $providers = Provider::with('owner:id,first_name,last_name,email')
            ->ofStatus(1)->ofApproval(1)->latest()->get();

        session()->forget('variations');

        return view('servicemanagement::admin.create', compact('categories', 'zones', 'providers'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $check = $this->validateUploadedFile($request, ['cover_image', 'thumbnail', 'gallery']);
        if ($check !== true) {
            return $check;
        }
        ...
        if ((int) $request->is_single_provider === 1 && $request->single_provider_mode === 'specific' && !empty($request->single_provider_id)) {
            $providerExists = Provider::where('id', $request->single_provider_id)
                ->where('is_active', 1)
                ->where('is_approved', 1)
                ->exists();
            if (!$providerExists) {
                return back()->withErrors(['single_provider_id' => translate('selected_provider_is_not_active_or_approved')])->withInput();
            }
        }
        ...
        /* Gallery images - admin panel "Gallery Image" block */
        if ($request->has('gallery')) {
            $galleryPaths = [];
            foreach ($request->file('gallery') as $image) {
                if ($image && $image->isValid()) {
                    $galleryPaths[] = file_uploader('service/', 'png', $image);
                }
            }
            $service->gallery = $galleryPaths;
        }

        $service->tax = $request->tax;
        $service->min_bidding_price = $request->min_bidding_price;
        $service->is_single_provider = (int) $request->is_single_provider === 1 ? 1 : 0;
        $service->single_provider_mode = $service->is_single_provider ? $request->single_provider_mode : null;
        $service->single_provider_id = null;
        if ($service->is_single_provider && $request->single_provider_mode === 'specific') {
            $service->single_provider_id = $request->single_provider_id;
        }
        // any_first: create par lock null — pehla subscriber claim karega
        $service->save();
    ...
    public function edit(string $id): View|Factory|RedirectResponse|Application
    {
        ...
        if (isset($service)) {
            ...
            return view('servicemanagement::admin.edit', compact('categories', 'zones', 'service', 'tagNames', 'providers'));
        }
    ...
    public function update(Request $request, string $id): JsonResponse|RedirectResponse
    {
        ...
        $request->validate([
            ...
            'gallery.*' => 'image|max:'. uploadMaxFileSizeInKB('image') .'|mimes:' . implode(',', array_column(IMAGEEXTENSION, 'key')),
        ...
        if ($request->hasFile('cover_image')) {
            $service->cover_image = file_uploader('service/', 'png', $request->file('cover_image'), $service->cover_image);
        }

        if ($request->hasFile('thumbnail')) {
            $service->thumbnail = file_uploader('service/', 'png', $request->file('thumbnail'), $service->thumbnail);
        }
        ...
        $service->gallery = array_values(array_filter($galleryPaths));

        $service->tax = $request->tax;
        $service->min_bidding_price = $request->min_bidding_price;

        $previousLock = $service->single_provider_id;
        $previousMode = $service->getOriginal('single_provider_mode');
        $service->is_single_provider = (int) $request->is_single_provider === 1 ? 1 : 0;
        $service->single_provider_mode = $service->is_single_provider ? $request->single_provider_mode : null;

        if (!$service->is_single_provider) {
            $service->single_provider_id = null;
        } elseif ($request->single_provider_mode === 'specific') {
            $service->single_provider_id = $request->single_provider_id;
        } else {
            // any_first: purana claim tab tak rakho jab tak pehle se any_first tha;
            // mode change hua to claim reset (dubara first subscriber lega)
            $service->single_provider_id = ($previousLock && $previousMode === 'any_first')
                ? $previousLock
                : null;
        }
