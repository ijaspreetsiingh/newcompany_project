...
class ServiceController extends Controller
{
    ...
    public function statusUpdate(Request $request): JsonResponse
    {
        ...
        $providerId = $request->user()->provider->id ?? null;
        if (!$providerId) {
        ...
        if ($request['status'] == 1) {
            ...
            // usi service ka exclusive lock ban jao
            Service::whereIn('sub_category_id', $request['sub_category_ids'])
                ->where('is_single_provider', 1)
                ->where('single_provider_mode', 'any_first')
                ->whereNull('single_provider_id')
                ->update(['single_provider_id' => $providerId]);
        } else {
            // Unsubscribe: apne any_first claims release karo (dubara pehla subscriber lega)
            Service::whereIn('sub_category_id', $request['sub_category_ids'])
                ->where('is_single_provider', 1)
                ->where('single_provider_mode', 'any_first')
                ->where('single_provider_id', $providerId)
