...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> login(
    ...
    if (response!.statusCode == 200 &&
      ...
      _saveDeviceTokenAndNavigate(
        response: response,
        phoneWithCountryCode: emailOrPhone,
        password: password,
        countryCode: countryCode,
      );
    } else if (response.statusCode == 401 &&
        response.body['response_code'] == "account_disabled_401") {
      showCustomSnackBar(
        response.body['message'].toString().capitalizeFirst ??
            response.statusText,
        toasterTitle: "account_blocked_notice",
  ...
  Future<void> _saveDeviceTokenAndNavigate({
    ...
    if (_isActiveRememberMe) {
      authRepo.saveUserNumberAndPassword(
