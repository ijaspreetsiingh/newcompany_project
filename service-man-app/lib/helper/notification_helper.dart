...
class NotificationHelper {
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      // ----- AUTO-ASSIGN: full-screen incoming booking popup -----
      else if(message.data['type']=='new_booking_request'){
        final String? bookingId = message.data['booking_id'];
        Get.find<BookingRequestController>().getBookingList(BooingListStatus.pending.name.toLowerCase(), 1);
        if(bookingId != null && bookingId != '' && !Get.currentRoute.contains(RouteHelper.incomingBookingPopup)) {
          Get.toNamed(RouteHelper.getIncomingBookingPopupRoute(bookingId: bookingId));
        }
      }
  ...
  static Future<void> showBigTextNotification({required String title, required String body, required String payload, required FlutterLocalNotificationsPlugin fln}) async {
    ...
    AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
      "jassbooking", AppConstants.appName, importance: Importance.max,
  ...
  static Future<void> showBigPictureNotificationHiddenLargeIcon(String title, String body, String playLoad, String image, FlutterLocalNotificationsPlugin fln) async {
    ...
    final AndroidNotificationDetails androidPlatformChannelSpecifics = AndroidNotificationDetails(
      "jassbooking", AppConstants.appName,
