...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  final FocusNode companyEmailFocus = FocusNode();
            ...
            builder: (signUpController){
              return Column(
                children: [
                  ...
                  SignUpStep1(formKey: step1FormKey, companyEmailFocus: companyEmailFocus, companyPhoneFocus: companyPhoneFocus,) : signUpController.currentStep == SignUpPageStep.step2 ?
  ...
  void validateStep1(SignUpController signUpController) async{
    ...
    if(step1FormKey.currentState!.validate() && signUpController.isLogoValid && signUpController.isCoverImageValidity){
      String phone = signUpController.countryDialCode + ValidationHelper.getValidPhone("${signUpController.countryDialCode}${signUpController.companyPhoneController.value.text}");

      signUpController.checkUserCredentials(signUpController.companyEmailController.text, phone).then((value){
        if(value.statusCode == 200){
          signUpController.updateRegistrationStep(SignUpPageStep.step2);
        }else{
          step1FormKey.currentState!.validate();
          if(signUpController.emailErrorText != null){
            companyEmailFocus.requestFocus();
          }else if(signUpController.phoneErrorText != null){
